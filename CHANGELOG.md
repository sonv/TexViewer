...
- **The documented Jinja configuration covers real `result` call shapes.**
...
- **`dev-api` now recognizes the same custom Markdown calls as `dev-md`.**
  Complete configured opener lines such as
  `result("theorem", "Green's theorem", "t-greens", card=true)` no longer
  remain visible when the matching Jinja boundary syntax is enabled.
...
- **Complex literal templates share a bounded matching budget per source
